...
% Estimates the maximum eigenvalue threshold (lambda_max) 
% based on best-fitting Marchenko–Pastur distribution
% Inputs:
%   Data   - (n x d) centered data matrix
%   lambda - vector of eigenvalues
% Output:
%   lambda_max - threshold eigenvalue for denoising

% Estimate standard deviation and aspect ratio
s = std(Data(:),'omitnan');
...
% Search over possible (sigma, ratio) pairs
for s_var = 1:50
    for r_var = 1:30
        counter = counter + 1;
...
f=f/sum(f) * length(lambda)/length(lamda_range); % Normalize histogram to match
