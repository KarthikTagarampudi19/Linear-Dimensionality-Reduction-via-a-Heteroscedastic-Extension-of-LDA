...
T = readmatrix('glass.data', 'FileType', 'text');
X_raw = T(:, 2:10);        % Features
Y = T(:, 11);              % Class labels
...
% Target dimension for HLDA
target_dim = 3;

% Variables to store final (last run) projections
Z_train_final = [];
Z_test_final = [];
Y_train_final = [];
Y_test_final = [];
...
for run = 1:n_runs
    ...
    if run == 1
        ...
        writematrix(sorted_eig_values(1:3), 'pca_first3_eigenvalues.csv');

        disp('First eigenvector:');
        disp(sorted_eig_vectors(:, 1)');
        writematrix(sorted_eig_vectors(:, 1), 'pca_first_eigenvector.csv');
    end
    ...
    % Save projection for final plots
    if run == n_runs
        mu_last = mu_train;
        coeff_last = coeff;
        W_last = para.W;
    ...
    writematrix(Z_train(1:3,:), 'hlda_first3_data_points.csv');

    % Classification (manual LDA/QDA)
...
% ==== PLOTS ====
plot_scatter_plain(Z_all, Y, 'Full HLDA Projection (Glass Dataset)');
...
function preds = manual_lda_classifier(X_train, Y_train, X_test)
    % Computes class means, pooled covariance, and scores for LDA classification
...
function preds = manual_qda_classifier(X_train, Y_train, X_test)
    % Computes class means, individual covariances, and scores for QDA classification
    classes = unique(Y_train);
    num_classes = numel(classes);
    n_features = size(X_train, 2);
    means = zeros(num_classes, n_features);
